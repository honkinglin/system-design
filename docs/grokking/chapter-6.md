...
我们的需求可以通过宽列存储数据库（如 [HBase](https://en.wikipedia.org/wiki/Apache_HBase)）轻松满足。HBase是一种面向列的键值NoSQL数据库，能够针对一个键存储多个列的值。HBase基于Google的 [BigTable](https://en.wikipedia.org/wiki/Bigtable) 模型，并运行在Hadoop分布式文件系统（[HDFS](https://en.wikipedia.org/wiki/Apache_Hadoop)）之上。HBase将数据分组存储到内存缓冲区中，当缓冲区满时，将数据转存至磁盘。这种存储方式不仅可以快速存储大量小数据，还可以通过键或行范围扫描快速检索数据。此外，HBase适合存储大小不一的数据，这也符合我们的服务需求。
