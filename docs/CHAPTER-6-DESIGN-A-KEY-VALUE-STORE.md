...
**步骤 1**：将键空间分为桶（在本例中为 4 个），如图 6-13 所示。每个桶用作根级节点，以保持树的深度有限。

![图6-13](/f6-13.png)

**步骤 2**：一旦桶创建完成，使用均匀哈希方法对每个桶中的键进行哈希处理（如图 6-14 所示）。

![图6-14](/f6-14.png)

**步骤 3**：为每个桶创建一个单独的哈希节点（如图 6-15 所示）。

![图6-15](/f6-15.png)

**步骤 4**：通过计算子节点的哈希值向上构建树，直到根节点（如图 6-16 所示）。
